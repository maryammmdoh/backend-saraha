...
async function bootstrap() {
    ...
    app.set("trust proxy", true); // Enable trust proxy if your app is behind a reverse proxy (e.g., Heroku, Nginx) to ensure that the client's IP address is correctly identified for rate limiting and other middleware that relies on the client's IP address. Adjust this setting based on your deployment environment and proxy configuration.
    // await sendEmail({
    ...
    app.use(rateLimit({
        ...
        limit : (req, res) => { // Custom function to determine the rate limit based on the client's IP address and geolocation information, this allows you to apply different rate limits for different clients based on their location (e.g., stricter limits for certain countries or regions) and also it makes both ipv4 and ipv6 work by normalizing the IP address format using the ipKeyGenerator function provided by express-rate-limit, this ensures that both IPv4 and IPv6 addresses are treated consistently for rate limiting purposes, preventing issues where the same client might be identified differently based on the IP version they are using 
            const geoInfo = geolite.get(req.ip) || {}; // Get geolocation information for the client's IP address using the geolite2 library, this can be used to apply different rate limits based on the client's location (e.g., stricter limits for certain countries or regions)
